...
(function() {
    ...
    var pinchCenterX = 0.5, pinchCenterY = 0.5; // نسب مئوية من العرض والارتفاع
    ...
    function buildFloatingPlayer() {
        ...
        container.addEventListener('wheel', function(e) {
            if (e.ctrlKey || e.metaKey) {
                e.preventDefault();
                const delta = e.deltaY > 0 ? -0.08 : 0.08; // حساسية أقل
                let newScale = Math.min(2.5, Math.max(0.4, currentScale + delta));
                // نحافظ على المركز
                const rect = container.getBoundingClientRect();
                const cx = rect.width / 2;
                const cy = rect.height / 2;
                const oldScale = currentScale;
                // نضبط الترجمة للحفاظ على مركز العنصر
                const dx = cx * (1 - newScale / oldScale);
                const dy = cy * (1 - newScale / oldScale);
                translateX += dx;
                translateY += dy;
    ...
    function updateTransform() {
        ...
        container.style.transform = `translate3d(${translateX}px, ${translateY}px, 0) scale(${currentScale})`;
    }
    ...
    function handleTouchStart(e) {
        ...
        if (touches.length === 1) {
            ...
            // حساب مركز التكبير كنسبة من عرض وارتفاع العنصر
            const rect = elements.container.getBoundingClientRect();
            const cx = (t1.clientX + t2.clientX) / 2;
            const cy = (t1.clientY + t2.clientY) / 2;
            pinchCenterX = (cx - rect.left) / rect.width;
            pinchCenterY = (cy - rect.top) / rect.height;
            // نحد من النسب بين 0 و 1
            pinchCenterX = Math.min(1, Math.max(0, pinchCenterX));
            pinchCenterY = Math.min(1, Math.max(0, pinchCenterY));
            startTranslateX = translateX;
            startTranslateY = translateY;
        }
        e.preventDefault();
    ...
    function handleTouchMove(e) {
        ...
        if (touches.length === 1 && isDragging) {
            const touch = touches[0];
            const rect = elements.container.getBoundingClientRect();
            const dx = touch.clientX - rect.left - startX;
            const dy = touch.clientY - rect.top - startY;
            translateX = startTranslateX + dx;
            translateY = startTranslateY + dy;
            updateTransform();
            e.preventDefault();
            ...
            if (initialPinchDist > 0) {
                ...
                let newScale = Math.min(2.5, Math.max(0.4, initialScale * scaleFactor));
                // حساب الترجمة الجديدة للحفاظ على نقطة التكبير ثابتة
                const rect = elements.container.getBoundingClientRect();
                const oldScale = currentScale;
                // إحداثيات نقطة التكبير بالبكسل نسبة للعنصر قبل التكبير
                const px = pinchCenterX * rect.width;
                const py = pinchCenterY * rect.height;
                // الإزاحة المطلوبة للحفاظ على النقطة
                const dx = px * (1 - newScale / oldScale);
                const dy = py * (1 - newScale / oldScale);
                translateX = startTranslateX + dx;
                translateY = startTranslateY + dy;
                currentScale = newScale;
                updateTransform();
