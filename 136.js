...
(function() {
    ...
    function buildFloatingPlayer() {
        ...
        container.addEventListener('wheel', function(e) {
            if (e.ctrlKey || e.metaKey) {
                ...
                // نحافظ على مركز العنصر أثناء التكبير
                const rect = container.getBoundingClientRect();
                const cx = rect.left + rect.width/2;
                const cy = rect.top + rect.height/2;
                // نحسب الترجمة الجديدة
    ...
    // دالة تحديث التحويلات (ترجمة + تكبير)
    ...
    function startDragMouse(e) {
        ...
        // نأخذ الترجمة الحالية كنقطة بداية
    ...
    function handleTouchStart(e) {
        ...
        if (touches.length === 1) {
            ...
            // مركز التكبير نسبة إلى العنصر
            const rect = elements.container.getBoundingClientRect();
            const cx = (t1.clientX + t2.clientX) / 2 - rect.left;
            const cy = (t1.clientY + t2.clientY) / 2 - rect.top;
            // نحول إلى إحداثيات داخل العنصر (مع مراعاة التكبير الحالي)
            pinchCenterX = cx;
            pinchCenterY = cy;
            startTranslateX = translateX;
            startTranslateY = translateY;
    ...
    function handleTouchMove(e) {
        ...
        if (touches.length === 1 && isDragging) {
            ...
            if (initialPinchDist > 0) {
                ...
                // نحسب الترجمة الجديدة للحفاظ على مركز التكبير
                const rect = elements.container.getBoundingClientRect();
                const currentWidth = rect.width;
                const currentHeight = rect.height;
                const oldScale = currentScale;
                const newWidth = currentWidth * (newScale / oldScale);
                const newHeight = currentHeight * (newScale / oldScale);
                // نضبط الترجمة بحيث يبقى مركز التكبير ثابتاً
                const dx = (newWidth - currentWidth) / 2;
                const dy = (newHeight - currentHeight) / 2;
                // نضيف فرق التكبير بالنسبة لمركز التكبير
                const cx = pinchCenterX;
                const cy = pinchCenterY;
                // الإزاحة الإضافية للحفاظ على النقطة
                const offsetX = cx * (1 - newScale / oldScale);
                const offsetY = cy * (1 - newScale / oldScale);
                translateX = startTranslateX + dx + offsetX;
                translateY = startTranslateY + dy + offsetY;
                currentScale = newScale;
                updateTransform();
                e.preventDefault();
            }
